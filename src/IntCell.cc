...
const APL_Integer iv = value.ival;
   ...
   if (iv < int_fact_max)   // integer result that fits into a 64-bit int
      {
        return Value::zI(Z, int_factorials[iv]);
      }

   if (iv < float_fact_max)   // float result that fits into a 64 bit double
      {
        return Value::zF(Z, float_factorials[iv]);
      }
...
APL_Integer result = 0;
   ...
   return Value::zI(Z, result);
}
//-----------------------------------------------------------------------------
ErrorCode
IntCell::bif_magnitude(Cell * Z) const
{
   if (value.ival >= 0)   Value::zI(Z, value.ival);
   else                   bif_negative(Z);
   return E_NO_ERROR;
}
//-----------------------------------------------------------------------------
   ...
   switch(value.ival)
      {
        ...
        case  1: return Value::z1(Z);
        case -1: return Value::z_1(Z);
      }

#ifdef RATIONAL_NUMBERS_WANTED
   if (value.ival < 0)   // make denominator positive
      return Value::zR(Z, -1, -value.ival);   /// 1 ÷ X = ¯1 ÷ -X
   else
      return Value::zR(Z, 1, value.ival);     // 1 ÷ X
#endif

   return Value::zF(Z, 1.0/value.ival);
}
//-----------------------------------------------------------------------------
ErrorCode
...
{
   return Value::zF(Z, M_PI * value.ival);
}
//-----------------------------------------------------------------------------
ErrorCode
IntCell::bif_pi_times_inverse(Cell * Z) const
{
   return Value::zF(Z, value.ival / M_PI);
}
//-----------------------------------------------------------------------------
ErrorCode
...
IntCell::bif_exponential(Cell * Z) const
{
   // e to the B-th power
   //
   return Value::zF(Z, exp(APL_Float(value.ival)));
}
//-----------------------------------------------------------------------------
ErrorCode
IntCell::bif_nat_log(Cell * Z) const
{
   ...
   if (value.ival > 0)
      {
        return Value::zF(Z, log(APL_Float(value.ival)));
      }
   ...
   if (A->is_integer_cell())   // integer A + integer B
      {
        ...
        if (sum_overflow(z, a, b))
           return Value::zF(Z, APL_Float(a) + APL_Float(b));
        else
           return Value::zI(Z,z);
      }
...
const bool A_zero = A->is_near_zero();
   if (b == 0)   // division by 0
      {
        if (A_zero)   return Value::z1(Z);   // 0÷0 is 1 in APL
        return E_DOMAIN_ERROR;
      }
   ...
   if (A->is_integer_cell())
      {
        ...
        if (b == gcd)   return Value::zI(Z, a/gcd);
        return Value::zR(Z, a/gcd, b/gcd);
#else
        const APL_Float i_quot = a / b;
        const APL_Float r_quot = a / APL_Float(b);

        if (a != i_quot * b)   return Value::zF(Z, r_quot);
        else                   return Value::zF(Z, i_quot);
#endif
      }
   ...
   if (A->is_integer_cell())
      {
        ...
        if (prod > LARGE_INT ||
            prod < SMALL_INT)   Value::zF(Z, prod);
        else                    Value::zI(Z, a * b);
        return E_NO_ERROR;
      }
...
const bool invert_Z = b < 0;
   ...
   if (A->is_integer_cell())
      {
        ...
        if (a == 0)
           {
             ...
             return Value::z0(Z);   // 0^b = 0
           }
        if (a == 1)    return Value::z1(Z);   // 1^b = 1
        if (a == -1)   // -1^b = 1 or -1
           {
              if (b & 1)   return Value::z_1(Z);   // -1^(2N+2) = -1
              else         return Value::z1(Z);    // -1^(2N+1) =  1
           }
        ...
        if (!overflow)
           {
             if (negate_Z)   zi = - zi;
             if (!invert_Z)   return Value::zI(Z, zi);

             if (zi != 0)   return Value::zF(Z, 1.0 / zi);
             return E_DOMAIN_ERROR;
           }
   ...
   //
   {
     ...
     if (!invert_Z)   return Value::zC(Z, z);

     if (denom == 0.0)   return E_DOMAIN_ERROR;
     return Value::zC(Z, z.real() / denom, - z.imag() / denom);
   }
}
//-----------------------------------------------------------------------------
ErrorCode
...
const APL_Float a = A->get_real_value();
   if (a == 0.0)   return Value::zI(Z, value.ival);
   ...
   //
   if (value.ival == 0)   return Value::z0(Z);
   ...
   if (qct != 0)
      {
        ...
        if (quot > qc - qct)   return Value::z0(Z);
        if (quot < qf + qct)   return Value::z0(Z);
      }
   ...
   if (A->is_integer_cell())
      {
        ...
        return Value::zI(Z, rest);
      }

APL_Float z = value.ival - a * qf;
   if (Cell::is_near_zero(z))   return Value::z0(Z);
...
IntCell::bif_maximum(Cell * Z, const Cell * A) const
{
   if (A->is_integer_cell())
      {
         const APL_Integer a = A->get_int_value();
         if (a >= value.ival)   return Value::zI(Z, a);
         return Value::zI(Z, value.ival);
      }
