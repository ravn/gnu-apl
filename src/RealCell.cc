...
const APL_Float b = get_real_value();
   ...
   switch(fun)
      {
        case -12:
              ComplexCell(0, b).bif_exponential(Z);   return E_NO_ERROR;

        case -11:
             return ComplexCell::zv(Z, 0.0, b );

        case -10:
             return FloatCell::zv(Z,       b );
               ...
               if (square < 0.0)   // complex square root
                  {
                    ...
                    if (b < 0.0)   return ComplexCell::zv(Z, 0.0, -root);
                    else           return ComplexCell::zv(Z, 0.0,  root);
                  }
               else           // real square root
                  {
                    const APL_Float root = sqrt(square);      // (¯1 + R⋆2)⋆0.5
                    if (b < 0.0)   return FloatCell::zv(Z, -root);
                    else           return FloatCell::zv(Z,  root);
                  }
             }

        case -7:
             if (b > -1.0 && b < 1.0)   return FloatCell::zv(Z, atanh(b));
             if (b == -1.0 || b == 1.0)   return E_DOMAIN_ERROR;
             return ComplexCell::do_bif_circle_fun(Z, -7, APL_Complex(b));

        case  -6:
              if (b > 1.0)   return FloatCell::zv(Z, acosh(b));
              return ComplexCell::do_bif_circle_fun(Z, -6, APL_Complex(b));

        case -5:
             return FloatCell::zv(Z, asinh(b));

        case -4:
             { const double arg = b*b - 1.0;
               if (arg >= 0.0)   return FloatCell::zv(Z, sqrt(arg));
               return ComplexCell::do_bif_circle_fun(Z, -4, APL_Complex(b));
             }
        case -3:
             return FloatCell::zv(Z, atan (b));

        case -2:
              if (b >= -1.0 && b <= 1.0)  return FloatCell::zv(Z, acos (b));
              return ComplexCell::do_bif_circle_fun(Z, -2, APL_Complex(b));

        case -1:
             if (b >= -1.0 && b <= 1.0)  return FloatCell::zv(Z, asin (b));
             return ComplexCell::do_bif_circle_fun(Z, -1, APL_Complex(b));

        case 0:
             {
               const APL_Float b2 = 1.0 - b*b;
               if (b2 >= 0.0)   return FloatCell::zv(Z, sqrt(b2));
               return ComplexCell::do_bif_circle_fun(Z, 0, APL_Complex(b));
             }

        case 1:
             return FloatCell::zv(Z, sin(b));

        case 2:
             return FloatCell::zv(Z, cos(b));

        case 3:
             return FloatCell::zv(Z, tan(b));

        case 4:
             return FloatCell::zv(Z, sqrt(1 + b*b));

        case   5: return FloatCell::zv(Z, sinh(b));

        case 6:
             return FloatCell::zv(Z, cosh(b));

        case 7:
             return FloatCell::zv(Z, tanh(b));
        ...
        case 10:
              if (b < 0.0)   return FloatCell::zv(Z, -b);
                  else           return FloatCell::zv(Z,  b);

        case 11:
             return FloatCell::zv(Z, 0.0 );

        case 12:
             return FloatCell::zv(Z, (b < 0.0) ? M_PI : 0.0);
      }
